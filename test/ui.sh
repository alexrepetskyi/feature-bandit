...
echo "2. cancelling a choice stops the run"
new_repo cancel
...
echo "3. a failing gum call stops the run"
new_repo gumfail
fb_run_tty "Accept them" "no localisation" "__FAIL__" > "$WORK/o" 2>&1
check "stopped"           "1" "$?"
check "spec not approved" "1" "$("$FB" status 2>&1 | grep -c 'Continues at: Specification')"

echo "4. gum missing while a terminal is in use"
new_repo nogum
rm -f "$BIN/gum"
...
echo "5. gum is not needed without a terminal"
new_repo nogum2
rm -f "$BIN/gum"
saved_path=$PATH
PATH="$(nogum_path)"
...
echo "6. an approval that was never given is not an approval"
new_repo failclosed
...
echo "11. Ctrl-C ends the running command, not just the runner"
new_repo interrupt
...
echo "12. the command's own exit code survives"
new_repo exitcode
...
echo "13. every block keeps its own complete log"
new_repo logs
...
echo "14. verification logs never overwrite each other"
new_repo verlogs
printf 'test:\n\t@echo tests pass\nlint:\n\t@echo no errors\n' > Makefile
git add -A && git commit -qm makefile
fb_run_happy > "$WORK/o" 2>&1
...
echo "15. a log that cannot be written stops the stage"
new_repo logfail
...
echo "16. resume reports where it is"
new_repo resumeinfo
